...
MongoClient.connect(connectionString, (err, client) => {
    ...
    const db = client.db('landclients')
    const titreCollection = db.collection('lands')
    ...
    app.post('/confirmation', (req, res) => {
        titreCollection.insertOne(req.body)
            .then(result => {
                console.log(result)
                res.render("client/index", {alert_msg: "We have received and are processing your request, come back in max 7 days to collect your certificate!"})
            })
            .catch(error => console.error(error))
    })

    app.get('/api/admin/addland', function (req, res) {

        // send data from database
        db.collection('lands').find().toArray()
        .then(results => {
            res.render('admin/addland', { results: results })
            console.log(results)
        })
        .catch(error => console.error(error))
      })
  ...
  app.get('/api/admin', function (req, res) {    
        res.redirect('/api/admin/lands');
  })


app.get('/api/admin/index', async function (req, res)  {
    try {
...
// Create a new file system based wallet for managing identities.
  ...
  // Create a new gateway for connecting to our peer node.
	...
	console.log(JSON.parse(result)[0]["Record"]);
        ...
        res.render("admin/lands",{ list:JSON.parse(result)});
} catch (error) {
...
app.post('/api/admin/addland', urlencodedParser, async function (req, res) {
    try {
const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
// Create a new file system based wallet for managing identities.
...
app.get('/api/query/:land_index', async function (req, res) {
    ...
    // Create a new file system based wallet for managing identities.
     ...
     // Evaluate the specified transaction.
            ...
            const result = await contract.evaluateTransaction('queryTitre', req.params.land_index);
            //titreNum["titreNumber"] = req.params.land_index;
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
            res.render("admin/land-detail",{ titre:JSON.parse(result)});
    } catch (error) {
            console.error(`Failed to evaluate transaction: ${error}`);
            res.status(500).json({error: error});
            process.exit(1);
...
app.put('/api/changelandowner/:land_index', async function (req, res) {
    try {
const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        const ccp = JSON.parse(fs.readFileSync(ccpPath, 'utf8'));
// Create a new file system based wallet for managing identities.
  ...
  // Create a new gateway for connecting to our peer node.
        ...
        const contract = network.getContract('titrecontract');
// Submit the specified transaction.
        await contract.submitTransaction('changeTitreOwner', req.params.land_index, req.body.fullName);
        console.log('Transaction has been submitted');
